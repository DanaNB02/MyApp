...
struct StorySentence2: Identifiable {
    let id = UUID()
    let text: String
    let isAngry: Bool
}

final class StoryViewModel2: ObservableObject {
    @Published var sentences: [StorySentence2] = [
        StorySentence2(text: "بحث غرابٌ عن الماء في صيفٍ حار.", isAngry: true),
        StorySentence2(text: "فوجد إناءً فيه قليل من الماء في القاع لا يستطيع الوصول إليه.", isAngry: false),
        StorySentence2(text: "لم يستسلم الغراب.", isAngry: false),
        StorySentence2(text: "بدأ بجمع الحجارة الصغيرة وأسقطها في الإناء.", isAngry: false),
        StorySentence2(text: "فارتفع مستوى الماء وهكذا شرب الماء بنجاح.", isAngry: false)
    ]
...
struct ElsaStory: View {
    @StateObject private var vm = StoryViewModel2()
    @State private var showEmojis = false

    var body: some View {
        ZStack {
            Color.elsacolor.ignoresSafeArea()
...
struct EmojiEmitter: View {
    @State private var particles: [EmojiParticle2] = []
    private let emojis = ["😠", ]
...
struct EmojiParticle2: Identifiable {
    let id = UUID()
    let char: String
    var x: CGFloat
    var y: CGFloat
